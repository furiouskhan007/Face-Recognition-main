...
def IOU(box1, box2, screen_size=(480, 640)):  # calculating IOU
    ...
    x_min1, y_min1, x_max1, y_max1 = int(x_min1), int(y_min1), int(x_max1), int(y_max1)

    x_min2, y_min2, x_max2, y_max2 = box2
    x_min2, y_min2, x_max2, y_max2 = int(x_min2), int(y_min2), int(x_max2), int(y_max2)

    # Adjusting coordinates to fit within screen size
    x_min1 = max(0, min(x_min1, screen_size[1] - 1))
    x_max1 = max(0, min(x_max1, screen_size[1] - 1))
    y_min1 = max(0, min(y_min1, screen_size[0] - 1))
    y_max1 = max(0, min(y_max1, screen_size[0] - 1))

    x_min2 = max(0, min(x_min2, screen_size[1] - 1))
    x_max2 = max(0, min(x_max2, screen_size[1] - 1))
    y_min2 = max(0, min(y_min2, screen_size[0] - 1))
    y_max2 = max(0, min(y_max2, screen_size[0] - 1))

    # Marking the boxes in the boolean arrays
    boolean_box1[y_min1:y_max1, x_min1:x_max1] = True
    boolean_box2[y_min2:y_max2, x_min2:x_max2] = True

    overlap = np.logical_and(boolean_box1, boolean_box2)  # Logical AND
    union = np.logical_or(boolean_box1, boolean_box2)  # Logical OR
